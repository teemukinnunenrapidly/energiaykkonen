...
const ERROR_PATTERNS: Array<{
  ...
  // Network errors
  { pattern: /fetch.*failed|network.*error|connection.*refused|failed.*to.*fetch/i, type: 'network', retryable: true },
  { pattern: /ECONNREFUSED|ENOTFOUND|ETIMEDOUT/i, type: 'network', retryable: true },
  { pattern: /ERR_NETWORK|ERR_INTERNET_DISCONNECTED/i, type: 'network', retryable: true },
  
  // Timeout errors
  { pattern: /timeout|timed.*out/i, type: 'timeout', retryable: true },
  { pattern: /request.*timeout|response.*timeout/i, type: 'timeout', retryable: true },
  
  // Permission/CORS errors
  { pattern: /CORS|cross.*origin|blocked.*by.*CORS/i, type: 'permission', retryable: false },
  { pattern: /403|forbidden|unauthorized|401/i, type: 'permission', retryable: false },
  
  // Configuration errors
  { pattern: /config.*not.*found|invalid.*config|failed.*to.*load.*config/i, type: 'config', retryable: true },
  { pattern: /JSON.*parse|syntax.*error.*unexpected/i, type: 'config', retryable: false },
  { pattern: /config\.json.*404|config\.json.*not.*found/i, type: 'config', code: 'CONFIG_404', retryable: true },
  
  // Dependency errors
  { pattern: /dependency.*not.*found|module.*not.*found|script.*not.*loaded/i, type: 'dependency', retryable: true },
  { pattern: /404.*\.js|404.*\.css|resource.*not.*found/i, type: 'dependency', retryable: true },
  
  // Validation errors
  { pattern: /validation.*failed|invalid.*data|schema.*error/i, type: 'validation', retryable: false },
  { pattern: /cards.*not.*found|no.*cards.*available/i, type: 'validation', code: 'NO_CARDS', retryable: false },
];

// HTTP status code to error type mapping
const HTTP_STATUS_ERRORS: Record<number, { type: ErrorType; retryable: boolean }> = {
...
class ErrorManager {
  ...
  classifyError(error: Error | string, context: Partial<ErrorContext> = {}): ErrorInfo {
  ...
  async retryOperation<T>(
    ...
    for (let attempt = 0; attempt < this.retryConfig.maxAttempts; attempt++) {
      try {
        ...
        console.log(`🔄 Attempt ${attempt + 1}/${this.retryConfig.maxAttempts}`, attemptContext);
        ...
        if (attempt === this.retryConfig.maxAttempts - 1 || !errorInfo.retryable) {
          console.error(`🚨 Operation failed permanently after ${attempt + 1} attempts`);
          throw errorInfo;
        }
  ...
  enableDebugLogging() {
    ...
    console.error = (...args) => {
      ...
      if (args[0] && typeof args[0] === 'string' && args[0].includes('E1 Calculator')) {
        const debugInfo = {
        ...
        try {
          const existingLogs = JSON.parse(sessionStorage.getItem('e1-debug-logs') || '[]');
          existingLogs.unshift(debugInfo);
          sessionStorage.setItem('e1-debug-logs', JSON.stringify(existingLogs.slice(0, 20)));
        } catch (e) {
...
export const loadJSONWithRetry = errorManager.loadJSONWithRetry.bind(errorManager);
export const loadResourceWithRetry = errorManager.loadResourceWithRetry.bind(errorManager);
