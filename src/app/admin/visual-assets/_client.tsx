...
export default function VisualAssetsClient() {
  ...
  useEffect(() => {
    ...
    if (selectedFolder) {
      filtered = filtered.filter(o => o.folder_id === selectedFolder);
    }
    if (searchQuery) {
  ...
  const renderFolderTree = (nodes: FolderNode[]) =>
    nodes.map(folder => (
      <div key={folder.id}>
        <div
          className={`flex items-center p-2 hover:bg-gray-100 rounded cursor-pointer ${selectedFolder === folder.id ? 'bg-blue-100' : ''}`}
  ...
  const handleDeleteObject = async (id: string) => {
    if (!confirm('Delete visual object?')) {
      return;
    }
    ...
    <>
      ...
      <div className="flex h-full">
        <div className="w-64 border-r bg-gray-50 p-4">
          ...
          <div className="space-y-1">
            <div
              className={`flex items-center p-2 hover:bg-gray-100 rounded cursor-pointer ${selectedFolder === null ? 'bg-blue-100' : ''}`}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button onClick={() => setIsCreateModalOpen(true)}>
                <Plus className="w-4 h-4 mr-2" /> Create Object
              </Button>
            </div>
          </div>
          ...
          >
            {filteredObjects.map(obj => {
              ...
              const imageUrl = getSafeImageUrl(
                firstImage?.cloudflare_image_id,
                'public'
              );
              return (
                <Card
                  ...
                  <CardHeader className="p-4 pb-2">
                    <div className="flex items-center justify-between">
                      <Badge variant="secondary" className="text-xs">
                        {obj.images.length}{' '}
                        {obj.images.length === 1 ? 'image' : 'images'}
                      </Badge>
      ...
      <CreateEditModal
        ...
        onSave={async (data, images) => {
          const obj = await createVisualObject({
            name: data.name,
            title: data.title,
            description: data.description,
            folder_id:
              data.folder_id === 'no-folder' ? undefined : data.folder_id,
            show_overlay: !!data.show_overlay,
          });
          if (images.length) {
            for (let i = 0; i < images.length; i++) {
              const url = await uploadToCloudflare(images[i]);
              await addImageToVisualObject(obj.id, {
                cloudflare_image_id: url.split('/').pop()!,
                title: images[i].name,
                display_order: i,
              });
            }
        ...
        onSave={async (data, images) => {
          ...
          if (data.imagesToDelete?.length) {
            for (const imgId of data.imagesToDelete) {
              await removeImageFromVisualObject(editingObject.id, imgId);
            }
          }
          if (images.length) {
            const existing =
