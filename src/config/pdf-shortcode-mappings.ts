...
export const pdfShortcodeMappings = {
  ...
  annualEnergyNeed: '[format:annual_energy_need:number:decimals=0,suffix= kWh/vuosi]',
  ...
  // New System Details
  electricityConsumption: '[format:heat_pump_consumption:number:decimals=0] kWh/vuosi',
  ...
  efficiencyNote: 'Arvio energiamäärästä, joka tarvitaan täyttämään laskennallinen energiantarve. Laskelmassa käytetty maltillista [COP_VALUE] hyötysuhdetta.',
  
  // Subsidy Note
  subsidyNote: '* ELY-keskuksen energiatuki öljylämmityksestä luopumiseen. Tuki on [format:ely_subsidy:currency] pientaloille. Edellyttää hakemuksen tekemistä ennen töiden aloittamista.',
  ...
  benefit3: 'Kotitalousvähennys 40% työn osuudesta (max [format:tax_deduction_max:currency])',
  ...
  infoBoxContent: 'Visiomme uusiutuvan energian tuottamisesta ydinvoimalan verran (4 TWh vuodessa) on kunnianhimoinen. Tämä vastaa 380 000 polttomoottoriauton vuosittaisia päästöjä. Vision taustalla on paljon laskelmia, ja siksi pidämmekin sen toteuttamista täysin mahdollisena.',
...
export const calculatedFields = {
  ...
  oil_consumption: (lead: any) => {
    ...
    return Math.round(lead.annual_energy_need / (oilEfficiency * oilEnergyContent));
  },
 ...
 */
export function resolvePDFFields(processor: any, lead: any): Record<string, string> {
