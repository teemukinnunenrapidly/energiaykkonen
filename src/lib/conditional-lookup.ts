...
import { getSessionField, getSessionCalculation, storeSessionField, getSessionSummary } from './session-data-table';
...
export async function processLookupShortcode(
  ...
  try {
    ...
    if (formData) {
      console.log(`🔍 [LOOKUP] Storing form data to session for lookup processing:`, formData);
    ...
    console.log(`🔍 [LOOKUP] Found table with ${lookupTable.conditions?.length || 0} conditions`);
    ...
    console.log(`🔍 [LOOKUP DEBUG] Session calculations available:`, calculations);
    console.log(`🔍 [LOOKUP DEBUG] Field keys:`, Object.keys(fields));
    console.log(`🔍 [LOOKUP DEBUG] Calculation keys:`, Object.keys(calculations));
    console.log(`🔍 [LOOKUP DEBUG] Specifically looking for 'valitse' field:`, getSessionField(sessionId, 'valitse'));
    console.log(`🔍 [LOOKUP DEBUG] Looking for energy calculation:`, calculations['Laskennallinen energiantarve (kwh)']);
    
    // Evaluate each condition in order
    for (const condition of lookupTable.conditions || []) {
      if (!condition.is_active) continue;
      
      console.log(`🔍 [LOOKUP] Evaluating condition ${lookupTable.conditions.indexOf(condition) + 1}: "${condition.condition_rule}"`);
      console.log(`🔍 [LOOKUP] Target shortcode: "${condition.target_shortcode}"`);
      ...
      if (result.result) {
        console.log(`✅ [LOOKUP] Condition matched! Returning: "${condition.target_shortcode}"`);
        return {
          success: true,
    ...
    console.log(`🔍 [LOOKUP DEBUG] Final session data summary:`, getSessionSummary(sessionId));
    
    const sessionFields = getSessionSummary(sessionId).fields;
    const debugInfo = Object.entries(sessionFields).map(([key, value]) => `${key}="${value}"`).join(', ');
...
function evaluateCondition(
  ...
  try {
    ...
    for (const ref of fieldReferences) {
      ...
      console.log(`🔍 [LOOKUP DEBUG] Looking for field '${fieldName}', got value:`, fieldValue);
      
      // If exact field name not found, try common field name mappings
      if (fieldValue === undefined) {
        const { fields } = getSessionSummary(sessionId);
        console.log(`🔍 [LOOKUP DEBUG] Field '${fieldName}' not found, trying field mapping...`);
        console.log(`🔍 [LOOKUP DEBUG] Available session fields:`, Object.keys(fields));
        
        // Try common field name variations for Finnish forms
        const fieldMappings: Record<string, string[]> = {
          'valitse': ['valitse_jompi_kumpi', 'valitse', 'heating_type', 'lämmitystyyppi'],
          'heating_type': ['valitse_jompi_kumpi', 'valitse', 'heating_type', 'lämmitystyyppi'],
        ...
        for (const possibleField of possibleFields) {
          ...
          if (testValue !== undefined) {
            console.log(`🔍 [LOOKUP DEBUG] Found field mapping: '${fieldName}' → '${possibleField}' = ${testValue}`);
      ...
      if (fieldValue === undefined) {
        console.error(`🔍 [LOOKUP DEBUG] Field '${fieldName}' not found in session data for sessionId: ${sessionId}`);
        return {
          success: false,
      ...
      const quotedValue = typeof fieldValue === 'string' ? `"${fieldValue}"` : fieldValue;
      processedCondition = processedCondition.replace(ref, quotedValue.toString());
    }
    ...
    for (const ref of calcReferences) {
      ...
      processedCondition = processedCondition.replace(ref, calcData.value.toString());
    }
    ...
    console.log(`🔍 [LOOKUP] Failed characters (if any):`, processedCondition.match(/[^\p{L}\p{N}\s"'=!<>()._-]/gu));
    ...
    console.log(`🔍 [LOOKUP] Condition result: ${result} (type: ${typeof result})`);
    ...
    if (processedCondition.includes('==')) {
      ...
      console.log(`🔍 [LOOKUP] Left side: ${parts[0]} (length: ${parts[0]?.length})`);
      console.log(`🔍 [LOOKUP] Right side: ${parts[1]} (length: ${parts[1]?.length})`);
    }
 ...
 */
function handleFallbackLookups(lookupName: string, sessionId: string): string | null {
  // Handle "Menekki" (consumption) - likely related to energy consumption
  if (lookupName.toLowerCase().includes('menekki')) {
    console.log(`🔍 [LOOKUP] Applying fallback for consumption-related lookup: "${lookupName}"`);
    ...
    if (heatingType) {
      if (heatingType.toLowerCase().includes('öljy') || heatingType.toLowerCase().includes('oil')) {
        return '[calc:oil-consumption]';
      }
      if (heatingType.toLowerCase().includes('sähkö') || heatingType.toLowerCase().includes('electric')) {
        return '[calc:electric-consumption]';  
      }
      if (heatingType.toLowerCase().includes('kaasu') || heatingType.toLowerCase().includes('gas')) {
        return '[calc:gas-consumption]';
      }
      if (heatingType.toLowerCase().includes('kaukolämpö') || heatingType.toLowerCase().includes('district')) {
