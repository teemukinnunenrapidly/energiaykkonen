 ...
 */
const autodiscoveredDependencies = new Map<
  string,
 ...
 */
export function storeSessionField(
  sessionId: string,
  fieldName: string,
  value: any
): void {
...
export async function processFormulaWithSession(
  ...
  try {
    ...
    for (const ref of fieldReferences) {
      ...
      console.log(
        `📋 [SESSION] Replaced "${ref}" with "${numericValue}": "${processedFormula}"`
      );
    }

    // Replace [lookup:xxx] references with conditional lookups
    const lookupReferences =
      processedFormula.match(/\[lookup:([^\]]+)\]/g) || [];
    ...
    for (const ref of lookupReferences) {
      ...
      console.log(
        `📋 [SESSION] Replaced "${ref}" with "${lookupResult.shortcode}": "${processedFormula}"`
      );
    }
    ...
    for (const ref of calcReferences) {
      ...
      console.log(
        `📋 [SESSION] Replaced "${ref}" with "${replacementValue}": "${processedFormula}"`
      );
    }
 ...
 */
export function evaluateProcessedFormula(processedFormula: string): {
  success: boolean;
  result?: number;
  error?: string;
} {
...
export function getSessionSummary(sessionId: string): {
  ...
  calculations: Record<string, SessionCalculationData>;
} {
  const table = getSessionTable(sessionId);
...
export function registerDependencies(
  calculationName: string,
  dependencies: { fields?: string[]; calculations?: string[] }
): void {
  ...
  console.log(
    `🔗 [DEPENDENCIES] Registered for "${calculationName}":`,
    dependencies
  );
}
 ...
 */
export function discoverDependenciesFromFormula(
  formulaName: string,
  formulaText: string
): void {
...
export async function discoverDependenciesFromLookup(
  ...
  try {
    ...
    for (const condition of lookupTable.conditions) {
      ...
      const fieldMatches =
        condition.condition_rule.match(/\[field:([^\]]+)\]/g) || [];
      fieldMatches.forEach(match => {
        const fieldName = match.replace(/\[field:([^\]]+)\]/, '$1').trim();
        fields.add(fieldName);
      });
      ...
      const calcMatches =
        condition.condition_rule.match(/\[calc:([^\]]+)\]/g) || [];
      calcMatches.forEach(match => {
        const calcName = match.replace(/\[calc:([^\]]+)\]/, '$1').trim();
        calculations.add(calcName);
      });
      ...
      if (condition.target_shortcode) {
        const targetFieldMatches =
          condition.target_shortcode.match(/\[field:([^\]]+)\]/g) || [];
        targetFieldMatches.forEach(match => {
          const fieldName = match.replace(/\[field:([^\]]+)\]/, '$1').trim();
          fields.add(fieldName);
        });

        const targetCalcMatches =
          condition.target_shortcode.match(/\[calc:([^\]]+)\]/g) || [];
    ...
    if (fields.size > 0 || calculations.size > 0) {
      ...
      console.log(
        `🔍 [DEPS] Auto-discovered for lookup "${lookupName}": fields=${Array.from(fields).join(', ')}, calcs=${Array.from(calculations).join(', ')}`
      );
    }
  } catch (error) {
    console.error(
      `Failed to discover dependencies for lookup ${lookupName}:`,
      error
    );
  }
}
 ...
 */
function invalidateDependentCalculations(
  sessionId: string,
  changedItem: string,
  ...
  if (dependents.length === 0) {
    console.log(
      `🔗 [DEPENDENCIES] No dependents found for ${itemType} "${changedItem}"`
    );
    return;
  }
  ...
  console.log(
    `🔄 [INVALIDATION] Queued for recalculation due to ${itemType} "${changedItem}":`,
    dependents
  );
}
 ...
 */
function findDependentCalculations(
  changedItem: string,
  itemType: 'field' | 'calculation'
): string[] {
    ...
    Object.entries(registry).forEach(([calcName, deps]) => {
      const shouldInvalidate =
        itemType === 'field'
          ...
          : deps.calculations.some(
              calc => calc.toLowerCase() === changedItemLower
            );
  ...
  autodiscoveredDependencies.forEach((deps, calcName) => {
    const shouldInvalidate =
      itemType === 'field'
        ? Array.from(deps.fields).some(
            field => field.toLowerCase() === changedItemLower
          )
        : Array.from(deps.calculations).some(
            calc => calc.toLowerCase() === changedItemLower
          );
 ...
 */
export function markCalculationCurrent(
  sessionId: string,
  calculationName: string
): void {
  const queue = invalidationQueue.get(sessionId);
