...
export function testDependencyTracking() {
  ...
  storeSessionCalculation(
    testSessionId,
    'Laskennallinen energiantarve (kwh)',
    9720,
    'kWh'
  );
  ...
  const needsRecalc1 = needsRecalculation(
    testSessionId,
    'Öljyn menekki vuodessa'
  );
  ...
  const needsRecalc2 = needsRecalculation(
    testSessionId,
    'Laskennallinen energiantarve (kwh)'
  );
  const needsRecalc3 = needsRecalculation(
    testSessionId,
    'Öljyn menekki vuodessa'
  );

  console.log(
    'Energy calculation needs recalc after field change:',
    needsRecalc2
  );
