...
export function calculateCO2Emissions(formData: PDFFormData): number {
  ...
  switch (heatingType) {
    ...
    case 'oil':
      ...
      const litersOfOil =
        energyNeed / PDF_CALCULATION_CONSTANTS.KWH_PER_LITER_OIL;
    ...
    case 'electric':
      co2PerYear =
        energyNeed * PDF_CALCULATION_CONSTANTS.CO2_PER_KWH_ELECTRICITY;
      break;
...
export const PDF_CALCULATIONS = {
  ...
  newSystem: {
    ...
    costYear1: (formData: PDFFormData): number => {
      ...
      const heatPumpConsumption =
        energyNeed / PDF_CALCULATION_CONSTANTS.HEAT_PUMP_COP;
      const annualCost =
        heatPumpConsumption * PDF_CALCULATION_CONSTANTS.ELECTRICITY_PRICE;
    ...
    co2PerYear: (formData: PDFFormData): number => {
      ...
      const heatPumpConsumption =
        energyNeed / PDF_CALCULATION_CONSTANTS.HEAT_PUMP_COP;
      return Math.round(
        heatPumpConsumption * PDF_CALCULATION_CONSTANTS.CO2_PER_KWH_ELECTRICITY
      );
    },
  },
  ...
  savings: {
    ...
    year10: (formData: PDFFormData): number => {
      const currentCost10 =
        PDF_CALCULATIONS.currentSystem.cost10Years(formData);
...
export function generatePDFCalculations(lead: any) {
  ...
  return {
    ...
    new_electricity_consumption:
      PDF_CALCULATIONS.newSystem.electricityConsumption(formData),
