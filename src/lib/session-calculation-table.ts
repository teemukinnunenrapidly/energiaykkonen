...
export function storeSessionCalculation(
  ...
  console.log(`📊 [SESSION TABLE] Stored: "${formulaName}" = ${value} ${unit || ''}`);
}
...
export function lookupSessionCalculation(
  ...
  if (entry) {
    console.log(`📊 [SESSION TABLE] Found: "${formulaName}" = ${entry.value} ${entry.unit || ''}`);
  } else {
 ...
 */
export function getSessionCalculationTable(sessionId: string): Record<string, CalculationEntry> {
  const sessionTable = sessionCalculations.get(sessionId);
...
export function clearSessionCalculations(sessionId: string): void {
  ...
  console.log(`📊 [SESSION TABLE] Cleared calculations for session: ${sessionId}`);
}
...
export function resolveExpressionFromSession(
  ...
  try {
    ...
    for (const ref of calcReferences) {
      ...
      resolvedExpression = resolvedExpression.replace(new RegExp(escaped, 'g'), entry.value.toString());
      console.log(`📊 [SESSION TABLE] Replaced "${ref}" with "${entry.value}": "${resolvedExpression}"`);
    }
    
    console.log(`📊 [SESSION TABLE] Final resolved expression: "${resolvedExpression}"`);
 ...
 */
export function evaluateMathExpression(expression: string): { success: boolean; result?: number; error?: string } {
  try {
