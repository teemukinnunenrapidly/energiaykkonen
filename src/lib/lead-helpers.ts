...
export function flattenLeadData(lead: Lead): Lead {
  ...
  return {
    ...
    current_energy_consumption: lead.form_data.current_energy_consumption || lead.current_energy_consumption,
    
    // Calculations
    annual_energy_need: lead.form_data.annual_energy_need || lead.annual_energy_need,
    heat_pump_consumption: lead.form_data.heat_pump_consumption || lead.heat_pump_consumption,
    heat_pump_cost_annual: lead.form_data.heat_pump_cost_annual || lead.heat_pump_cost_annual,
 ...
 */
export function createLeadFromFormData(formData: Record<string, any>, calculations: Record<string, any>): Partial<Lead> {
  // Define which fields are fixed columns (not in JSONB)
  const fixedFields = [
    'first_name', 'last_name', 'sahkoposti', 'puhelinnumero',
    'status', 'notes', 'annual_savings', 'five_year_savings', 
    'ten_year_savings', 'pdf_url', 'pdf_generated_at',
 ...
 */
export function buildJSONBFilter(query: any, field: string, operator: string, value: any): any {
