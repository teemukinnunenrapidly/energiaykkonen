...
import { DEFAULT_LOOKUPS, LookupContext, Metrics, StrategyDefinition } from './types';
...
const STRATEGIES: StrategyDefinition[] = [OilWoodMixedStrategy, OilStrategy, GasStrategy, WoodStrategy];

export function pickStrategy(n: LeadNormalized): StrategyDefinition {
  return STRATEGIES.find(s => s.matches(n)) || OilStrategy;
}

export function computeMetrics(n: LeadNormalized, lookups: LookupContext = DEFAULT_LOOKUPS): Metrics {
  ...
  const newElectricityKWh = Math.round((n.laskennallinenenergiantarve || 0) / 3.8);
  const newCostYear1 = Math.round(newElectricityKWh * (lookups.electricityPrice || 0.15));
  const newCost = {
    year1: newCostYear1,
  ...
  const newCo2 = Math.round(newElectricityKWh * (lookups.co2.electricityPerKWh || 0.181));
