...
export function normalizeLead(input: Record<string, any>): {
  ...
  for (const [alias, canonicalKey] of Object.entries(aliasMap)) {
    if (
      ...
      canonical[canonicalKey] === undefined
    ) {
  ...
  const normalized: LeadNormalized = {
    ...
    rakennusvuosi: data.rakennusvuosi
      ? parseInt(String(data.rakennusvuosi))
      : null,
    henkilomaara: data.henkilomaara ?? null,

    lammitysmuoto: data.lammitysmuoto ?? null,
    kokonaismenekki:
      data.kokonaismenekki !== undefined
        ? parseFiNumber(data.kokonaismenekki)
        : null,
    menekinhintavuosi:
      data.menekinhintavuosi !== undefined
        ? parseFiNumber(data.menekinhintavuosi)
        : null,
