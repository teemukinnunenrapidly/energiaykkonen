...
async function checkCardFields() {
  try {
    ...
    if (fields && fields.length > 0) {
      ...
      Object.entries(fieldsByCard).forEach(([cardId, cardFields]) => {
        ...
        cardFields.forEach(field => {
          console.log(`  - field_name: "${field.field_name}" | label: "${field.label}" | type: ${field.field_type}`);
        });
      });
      ...
      console.log('=======================');
      const uniqueFieldNames = [...new Set(fields.map(f => f.field_name))].sort();
    ...
    if (cards) {
      ...
      console.log('===============');
      cards.forEach(card => {
        console.log(`  - ${card.name} (${card.type}) - ID: ${card.id.substring(0, 8)}...`);
      });
    }
