...
class SecurityValidator {
  ...
  static async createSecurityHeaders() {
    ...
    const htaccessPath = path.join(CONFIG.paths.cache, '.htaccess');
    const htaccessContent = `# E1 Calculator Cache - Local Development Friendly
...
# Basic security - block PHP execution
<Files "*.php">
  Order deny,allow
  Deny from all
</Files>

# Optional: Add security headers if mod_headers is available
<IfModule mod_headers.c>
  Header always set X-Content-Type-Options "nosniff"
  Header always set X-Frame-Options "SAMEORIGIN"
</IfModule>

# Optional: Add caching if mod_expires is available
