...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('#test-connection-btn').on('click', function() {
            ...
            $.ajax({
                ...
                error: function() {
                    $result.html(
                        '<div class="notice notice-error"><p>' + 
                        e1_calculator_admin.strings.error + ' Connection test failed' + 
                        '</p></div>'
                    );
                },
                complete: function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $result.html(
                            '<div class="notice notice-success"><p>' + 
                            e1_calculator_admin.strings.success + ' ' + response.data.message + 
                            '</p></div>'
                        );
                        ...
                        $result.html(
                            '<div class="notice notice-error"><p>' + 
                            e1_calculator_admin.strings.error + ' ' + response.data.message + 
                            '</p></div>'
                        );
                    }
                },
                error: function() {
                    $result.html(
                        '<div class="notice notice-error"><p>' + 
                        e1_calculator_admin.strings.error + ' Sync failed' + 
                        '</p></div>'
                    );
                },
                complete: function() {
        ...
        $('#restore-backup-btn').on('click', function() {
            if (!confirm('Are you sure you want to restore the latest backup? This will replace the current widget files.')) {
                return;
            }
        ...
        $('#e1_calculator_api_key').on('focus', function() {
            $(this).attr('type', 'text');
        }).on('blur', function() {
